...
///
/// The high byte selects the op: `0x06 → Add` (ADDI), `0x04 → Sub` (SUBI). (Later commits extend this by one
/// high byte each — `0x02 → And` ANDI, `0x00 → Or` ORI, `0x0A → Eor` EORI.) The size field `ss = (opcode >>
...
/// single points (0x06/0x04 have none, but the guard is uniform across the family).
#[inline]
...
fn decode_dispatch(regs: &Registers) -> MicroState {
    ...
    // byte via [`imm_class`]; ADDI (0x06) and SUBI (0x04) are admitted this commit (the classifier returns
    // `None` for the other high bytes until later commits). The destination is data-alterable only (`Dn` (0) or the seven
