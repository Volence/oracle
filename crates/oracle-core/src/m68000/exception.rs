...
/// Push the **interrupt** exception frame — the same final layout and `s S s` write order as
/// [`push_trace_frame`], but with the interrupt-acknowledge (`ni`) cycle **interleaved between the PCL write
/// and the PCH/SR writes** (Yacht L1549: `... ns ni n- n nS ns ...`). Pinned to that stream: `SSP -= 6`;
/// **PCL @ B+4**; the [`MicroOp::IntAck`] CPU-space acknowledge; a mid idle (`n- n` = 4 cyc); **PCH @ B+2**;
/// **SR @ B+0**. The vector is autovectored (24 + `level`, generated by the recipe), so `IntAck` only places
/// the level on the bus — no external vector-number read. `level` sizes the IACK address.
pub fn push_interrupt_frame(
    ...
    // SSP -= 6 — A7 now points at the new stack top (B).
    buf.push(MicroOp::AdjustAddr { reg: 7, delta: -6 });
    // PCL @ B+4 (written FIRST, before the IACK).
    buf.push(MicroOp::EaCalc {
        base: Operand::AddrReg(7),
        index: Operand::WordStep,
        disp: Operand::WordStep,
    ...
    buf.push(MicroOp::Write {
        ...
        fc: Fc::Data,
        size: Size::Word,
        value: Operand::Scratch(saved_pc_slot),
    });
    // The interrupt-acknowledge (`ni`) cycle, interleaved between PCL and PCH/SR (Yacht L1549).
    buf.push(MicroOp::IntAck { level });
    // The mid idle (`n- n` = 4 cyc) between the IACK and the PCH/SR writes.
    buf.push(MicroOp::Internal { cycles: 4 });
    // PCH @ B+2 (written after the IACK).
    buf.push(MicroOp::EaCalc {
        base: Operand::AddrReg(7),
        index: Operand::Zero,
        disp: Operand::WordStep,
    ...
    buf.push(MicroOp::Write {
        ...
        fc: Fc::Data,
        size: Size::Word,
        value: Operand::ScratchHi16(saved_pc_slot),
    });
    // SR @ B+0 (written LAST — completing `s S s`).
    buf.push(MicroOp::Write {
        addr: Operand::AddrReg(7),
        fc: Fc::Data,
        size: Size::Word,
