...
//!
//! SY-1 scope: **PSG only**. FM writes are decoded off the same stream but not yet synthesized (that is
//! SY-2). Output is native `sample_rate` Hz stereo `i16` (mono PSG duplicated to both channels), a fixed
//! `sample_rate / 60` samples per NTSC frame.
...
pub struct AudioSink {
    ...
    /// The hand-rolled SN76489 synthesizer (SY-1's only sound source).
    psg: Sn76489,
...
impl AudioSink {
    ...
    fn render_frame(&mut self) {
        for _ in 0..self.samples_per_frame {
            let s = self.psg.next_sample();
            // PSG is mono on the Genesis → duplicate to both output channels.
            self.out.push(s);
            self.out.push(s);
        }
    }
...
impl BusEventSink for AudioSink {
    ...
    fn on_event(&mut self, e: BusEvent) {
        ...
        match e.addr {
            ...
            // FM windows are decoded off the same stream but not synthesized in SY-1 (that is SY-2).
            _ => {}
        }
    }
}
