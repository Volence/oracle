...
> `$defs/displayMask` (7: `type`, the four-name `items.enum`, `uniqueItems`, `description`), the key on
> `screenshot` and `scanlines` (3 each: a `required` entry, `$ref`, `description`; M4), and on
> `state_hash` the key (2) plus a `dependentRequired` over `framebuffer`, `framebufferSource` and
> `displayMask` (4; D1 with R1). The seven changed values are all prose: `state_hash`'s `$comment` (M3,
> *"VDP state only"* leaves the schema), its `framebuffer` `$comment` and its `caveat` description;
> `read_vram`'s `$comment`; `read`'s `caveat` description (whose pointer to "the row's prose" dangled
> until the §11.49 peek bullet existed); and `screenshot`'s and `scanlines`' `source` descriptions (the
> mask as a second reason for `stateRender`, stated as a MAY).
>
> **There is no red-free ordering in either direction, and it was measured rather than argued.** With
> these bytes and the six pins in place and the serve NOT applied, `schema_conformance` stayed green
> (step 0 accepted the pins, and the vectors passed and failed as declared, cases 5, 15 and 17
> included, because those three are *refusals*). The failure came from `Client::recv`, which judges
> every live reply against these fragments closed: 17 wire tests red (`layers` 6, `methods` 4,
> `scanlines` 7), every refusal reading `"displayMask" is a required property` (on scanlines,
> screenshot and state_hash replies alike). That is how this suite turns the hub's *"cases 5, 15 and 17
> are today's replies"* into a failure: not through the vectors, which judge documents, but through
> every live reply of those shapes. The other direction is §8 item 20's closure, which refuses a
> `displayMask` the old fragments do not declare. So the re-vendor and the serve are one commit, and no
> intermediate commit is red. The hub reported the contract gate green at `0937079`: 126 pass-vectors,
> 187 red, 77 closure checks.

> **⚑ (The previous copy, §11.48, F-DEBUGREAD-BANKED.) Three description strings, and the serve merged with it.**
...
> | leaf path | old | new |
> |---|---|---|
> | `methods["emulator/z80_read"].params.properties.len.minimum` | `0` | `1` |
> | `methods["emulator/z80_read"].params.properties.len.description` | *(unchanged prefix)* | + a `FLOOR RAISED 0->1 2026-09-09 (§11.45, oracle's F-Z80READ-LEN0-UNSERVEABLE)` clause |
> | `methods["emulator/z80_read"].result.properties.len.minimum` | `0` | `1` |
> | `methods["emulator/z80_read"].result.properties.len.description` | `"Bytes actually returned."` | `"Bytes actually returned. Floored at 1 with the request field (§11.45)."` |
>
> `$defs/hex` is **unchanged** (`^0x[0-9A-Fa-f]+$`), which is the half of the ruling worth stating: of the
> two fixes offered when this repo registered the divergence, upstream took the floor and not the pattern,
> so the empty blob stays unspellable and 0 becomes a refusal like every other `len`'s.
>
> **The re-vendor and the handler change are one commit, and there is no red-free ordering the other way.**
> `tests/request_bounds.rs` derives its probes from these bytes: vendor without the handler and its
> in-bounds control stops probing `len: 0` while `the_registered_unserveable_bound_is_still_live` still
> registers it; change the handler without vendoring and the same row's probe sends a `len: 0` the
> fragment still calls legal and the server now refuses by name. Both directions are red.
>
> **⚑ A large encoding churn passed through this file upstream and was reverted before we took it.**
> Between `5f66cc2` and `47e77ec` the schema was re-serialised to ASCII — 427 lines changed, 314 carrying
> `\u00XX` escapes where literal `§`, `—`, `–` had been — and `47e77ec` restored the raw UTF-8,
> content-identical. Vendoring at `59d29ac` therefore takes the de-escaped bytes and the **raw text diff
> against the copy this replaces is 4 added / 4 removed lines**, not 427. The normalized comparison above
> was run anyway and doubles as the control: had the size of the raw diff and the size of the semantic
> delta disagreed, the pin would have been wrong.
...
> `887d01c7` carry §11.41 **and** §11.40 (CR-Q): a whole new event fragment, `emulator/machineReplaced`,
> with four vectors. That is stated here rather than absorbed, because a re-vendor whose delta is larger
> than its dispatch said is exactly the case the structural diff exists to surface.
>
> **The §11.40 half is inert here and that is checked, not assumed.** `emulator/machineReplaced` is a
...
> fragment with no `METHODS` row would put the name in `SCHEMATIZED_NOT_ADVERTISED`, which is pinned by
> name. There is no red-free ordering in either direction, so the serve and the re-vendor are one commit.

*(Retired one step, kept because it is the previous re-vendor's own record. Everything below this line
until the vectors table describes `c5638e6e`, not the current copy.)*

> **⚑ This re-vendor moved BOTH blobs, as the one before it did, and for the same reason: §11.39 adds
> REQUIRED leaves, and a required leaf is exactly what a vector can witness.** Four vectors — a
> `restore` pass and a `restore` fail on absence, an event pass and an event fail on absence. So both
> files were re-copied from the object store at `c5638e6`, both `pin.*` triples moved, and both
> **Current copy** tables above and below were re-derived from the bytes actually written in this commit.
>
> **The expected delta was stated in advance and it held exactly.** The dispatch named it as
> *"`hitsDropped` added to `emulator/restore`'s result **and to the `emulator/romReloaded` event**, plus
> four vectors"*, with an instruction to STOP on anything else. The structural diff (leaf-path sets
> differenced, leaf values compared) found **9 added paths, 1 relocated, 0 changed values** in the schema
> and **4 added cases, 0 removed** in the vectors, with **exactly two** fragments changed in the whole
> document (`methods["emulator/restore"]` and `events["emulator/romReloaded"]`) and every top-level
> subtree outside them byte-identical. Recorded because a clean result is only informative if the check
> that would have caught a dirty one was actually run.
>
> **The one shape the diff surfaced that the sentence above does not, said out loud rather than filed
> under "nothing else".** `emulator/restore.result` was a **bare `{"$ref": "#/$defs/replyFields"}`** and is
> now `{"allOf": [{"$ref": …}], "required": [...], "properties": {...}}`. Read as a leaf-path set that is
> one path removed, which is why the count above says "9 added, 1 relocated" rather than "8 added, 0
> removed": the `$ref`'s *value* is unchanged and it still applies, so nothing is removed in §11.18's
> additivity sense — but a reader who saw only "0 removed" would not know the fragment changed shape.
> `tests/common/schema.rs` names the two fragments that are still bare `$ref`s (`emulator/release_all`,
> `emulator/log_clear`), because `restore` was its worked example until today.
>
> **What the added leaves mean for validation force, said plainly.** These are not prose. `restore`'s
> `result.required` grows from nothing to `["hitsDropped"]` and `romReloaded`'s `params.required` from
> `["path"]` to `["path","hitsDropped"]`, so a server that does **not** yet emit the key fails both
> fragments — which is the point, as it was for `symbolsDropped` and for §11.38. That is why the
> re-vendor and the serve land in **one commit**: there is no red-free ordering in either direction.
>
> ⚑ **This is the first re-vendor here to move an EVENT fragment's `required`.** Every line the test
> client receives is validated in `Client::recv`, notifications included, so from these bytes onward a
> `romReloaded` without `hitsDropped` is refused on the wire in every test that reloads a ROM — not only
> in the row that asserts it.
...
> already the vendored value — §11.45 changed a fragment's bounds and added no case, and no existing
> `emulator/z80_read` vector sends `len: 0` (the six that name the method use 16, 8193, a missing `addr`,
> a numeric `addr`, and two `len: 2` results), so the vector set is still consistent with the raised floor.
> That was checked by parsing this copy, not assumed from the blob equality.
>
> *(Retired one step.)* > **⚑ These bytes did not move on this re-vendor, and only the pin's revision did.** Blob `136bb716` was
...
> `git merge-base --is-ancestor 887d01c7 origin/main` exited 0. The recipe was run **once per path**
> rather than assumed to give one answer for two files. This is the fourth divergence in the last six
> re-vendors and the reason the recipe resolves the path rather than the tip.
>
> ⚑ **The dispatch that ordered this re-vendor named ONE file.** It said *"Our vendored copy is
> `crates/oracle-aether/tests/contract/bus-protocol.schema.json`"* and gave a blob comparison for that
> path alone. `vectors.json` moved too, and the two pins are asserted to name the **same revision** by
> step 0 of the gate, so re-vendoring the schema alone would have been red on its face. The recipe in
> this file is the authority and it says once per path; a dispatch naming a subset of the paths is not a
> licence to move a subset of the pins.
>
> *(Retired one step, kept because it is the previous re-vendor's own reading.)* ~~The tip and the recipe
> AGREED, for both paths, for the second re-vendor running.~~ That was `c5638e6e`, the second of the two
> agreements.
>
> *(Retired two steps, kept because it is that re-vendor's own reading.)* ~~The tip and the recipe
> AGREED, for both paths, after three of the last four re-vendors where they did not.~~ That was
> `75b16d02`, and it was the first of the two agreements.
>
> *(Retired, kept because it is the previous re-vendor's own reading.)* ~~The tip diverged from the recipe
> AGAIN — the third time in the last four re-vendors, and again a lane-log commit.~~ That was `2208aa8`
> against tip `4ee5b42b`.
>
> *(Retired, kept because it is the previous re-vendor's own reading.)* ~~The tip diverged from the recipe
...
> alone.
>
> ⚑ **This re-vendor lands in the SAME COMMIT as the serve it describes**, and §11.39 is §11.38's case
> exactly: not a convention but the *only* ordering with no red window, in **both** directions —
>
> * **Schema first, serve later** — `hitsDropped` is `required` on `emulator/restore`'s result and on the
>   `emulator/romReloaded` event's params, so every restore reply and every reload notification the suite
>   validates is refused the moment the new bytes land. That is deliberate upstream (§11.38's additivity
>   note, which §11.39 inherits: *"a server not yet serving them fails the fragment: that is the point"*),
>   and it means the schema cannot be parked ahead of the code.
> * **Serve first, schema later** — §8 item 20's closure rejects any result key the fragment does not
>   declare, so a reply carrying `hitsDropped` against the old schema is refused too.
>
> The event half widens the blast radius rather than narrowing it, and that is worth knowing before
> reading a red run: `Client::recv` validates **every** line, notifications included, so with the new
> bytes and no serve it is not one row that fails but every test that reloads a ROM.
>
> *(The line this replaces belonged to §11.38, where the third face of the window was the reset row
> `the_key_set_is_exact` naming that reply's key set exhaustively; and before that to the §11.36
> `lookup_equate` re-vendor, where the teeth were `params_closure.rs` and the `UNCOVERED_METHODS`
> coverage pin instead.)*

**Taken from the object store at a committed revision** — `git show c5638e6:contract/schema/bus-protocol.schema.json` and `git show c5638e6:contract/schema/tests/vectors.json` — never copied out of the sibling working tree. Both adoptions were then checked **by content address**: `git hash-object` on the written files returns `856e5b31…` and `75927e4f…`, each equal to `git rev-parse c5638e6:<path>` resolved in the *contract* repo. That is the one check that cannot be talked into agreeing, and this repo has caught a doctored restore with it before.
...
revisions, 2026-08-22..2026-09-06); 97/97 including side branches. **The load-bearing number is not
that 100%** — a content-addressed comparison fires exactly when the content differs, so a high rate is
what correctness looks like here — **it is the vacuity control**: the same comparison asked at
`pin.revision` instead of at tip fires **0 of 97**, and that is the figure that could have come back
non-zero. On false positives, two classes travel under one name and only one has a denominator: the
literal *touched-without-moving-its-blob* class is **0 of 0 and therefore UNMEASURED, structurally** —
git records a path in a commit's diff only when its blob or mode changes, so that denominator was never
going to be anything but zero, in this repo or any other, and it is covered by construction in
`tools/test_contract_drift_report.py` rather than claimed as measured. The class that actually risks
crying wolf for a **byte** pin is the semantically-null byte change, and that one **is** measured at
**1 of 97**. What that measurement does **not** cover it says itself: `git log
...
(runner: `tools/run_contract_drift_tests.sh`, 20 rows) instead. That one measured false positive is
`47e77ec`, *"restore raw UTF-8 (content-identical)"*, blob `f0ab1756` → `da37da70`: it moved bytes
without moving the parsed document. The report labels it **semantically null** rather than either
...
### What this re-vendor adopted — §11.41 (CR-R): `read_vdp_registers` leaves the BLOCKED set (2026-09-05)

**A whole method fragment, for the first time since `write_vram`, and it arrives by SUBTRACTION.**
`emulator/read_vdp_registers` has been a catalogued §6 row since the catalog was written. It was never
missing; it was **blocked**, by audit D-20, because its own row spelled its result with two literal
ellipses (`decoded{hintEnabled,…}`, `status{raw,vintPending,…}`) and left `raw[]`'s length unstated. A
fragment could not be written from it without inventing names, and §2.5 reads an absent fragment correctly
as *"not yet transcribed"* while a partial one would assert a completeness it does not have.

§11.41 removes both ellipses by **striking**, not by enumerating: `decoded{}` is gone entirely and
`status` is reduced to its one `raw` word. So the fragment is now writable with nothing invented, and the
BLOCKED set goes **5 → 4** (`z80_registers`, `read_vsram`, `call_stack`, `log_tail` remain). Like §11.25,
this is a row LEAVING the blocked set, not a row joining the catalog: no method is added or removed.

**The shape, and the one place the adopted text differs from the CR this lane wrote.** `raw[]` is exactly
24 entries, index-ordered 0 to 23, each a two-hex-digit **`0x..`-prefixed** string; `status` is an object
with one REQUIRED key, `raw`, a four-hex-digit `0x....` word, closed with `unevaluatedProperties: false`.
The CR at `docs/proposed/2026-09-05-cr-r-vdp-register-readback.md` said *"24 two-hex-digit strings"* and
cited D9 category 1 without ever writing the prefix out; M1 spells it. That is an under-specification
made explicit rather than a contradiction — both texts cite the same D9 category, and the served form is
the schema's `^0x[0-9A-Fa-f]{2}$` either way — but the fragment is what the handler is written against.

**`caveat` is declared ABSENT** (M3): the hazard that these 24 registers are write-only on real hardware
travels in the fragment's own `description`, on §2.4's ground that a caveat every reply carries is one
nobody reads. Emitting one here fails item 20's closure, which is the intended alarm.

**§8 gains item 29, and it is the half a shape cannot check.** The method must be a PEEK: calling it any
number of times on a paused machine leaves `state_hash.combined` byte-identical, leaves the control-port
write-pending toggle where it was, and leaves the sprite-overflow and collision latches set if they were
set. A real `$C00004` read clears all three. That obligation is invisible to a validator — a reply that
corrupted the machine on its way out would be perfectly conformant — so it lives in
`tests/vdp_registers.rs`, and the mechanism that makes it hold is `System::vdp` returning `&Vdp` while
`Vdp::control_read_status` takes `&mut self`.

> **⚑ A drafting miss travels in these bytes, and it is upstream's, so it is carried in the open rather
> than patched here.** The schema's top-level `description` at `887d01c7` still reads *"FIVE §6 ROWS
> REMAIN UNSCHEMATIZED AND DELIBERATELY SO — z80_registers, read_vdp_registers, read_vsram, call_stack,
> log_tail"* — in the same document that now carries `methods["emulator/read_vdp_registers"]`. §11.41's
> own ruling says the set goes *"5 to 4"* and names the four that remain, so the intent is not in doubt;
> the prose was not updated with the fragment.
>
> **This is the exact shape the `a0c50a11` entry below records** (that one left the description saying
> *"eight"* when the set had just become five), and it is handled the same way: **reported upstream, not
> hand-corrected**, because a vendored copy that is locally edited is a copy whose blob check is
> worthless. It is also harmless in the only way that matters — the description is prose, and the count
> is DERIVED by `contract/schema/tests/validate_contract_schema.py`, which parses `methods` and diffs it
> against §6. The gate is right; the sentence is stale. Nothing in this repo reads that sentence.

### What the re-vendor before this one adopted — §11.39 (CR-P): `hitsDropped` on `restore` and on the `romReloaded` EVENT (2026-09-05)

**Two required leaves again, on one method fragment and — for the first time in this file — one EVENT
fragment, plus four vectors.** §11.39 is **this lane's own CR**, and it exists only because the §11.38
serve banked a principle and the parcel that banked it asked the next question: *a principle gets applied
to the artifact in front of you and not to its neighbour, so enumerate what it covers before stopping.*
Enumeration found three neighbours. `emulator/restore` rewinds the frame counter exactly as `reset` does,
so hits recorded after the capture point describe a **discarded future**. The `romReloaded` event carried
no count, so a client that learns of reloads by *listening* — as opposed to calling — never saw one. And a
breakpoint's `hits` spans every boundary, which needed ratifying rather than changing. Adopted **as
proposed** on all three. Reviewer named in the ruling: **aeon**, in the same drive that reviews CR-O.

⚑ **The ruling states its own limit, and it belongs here rather than in a reader's head.** This server
emits exactly three events (`stopped`, `resumed`, `romReloaded`). There is no reset event and no restore
event, so the event half closes **one boundary of three** for a listening client: after this lands, a
listener still cannot learn that a reset or a restore happened, by any route. Event coverage for those two
is NOT proposed and NOT adopted — no consumer has asked for it. A reader who sees the event vector and
concludes listeners are covered is wrong.

⚑ **And the ruling's (c) is a NEGATIVE that this file's green cannot witness at all.** §11.39 ratifies,
for all four artifacts at once, that a **RECORD** with epoch-relative fields is dropped while an
**AGGREGATE** over an observer's life is **kept**: a breakpoint's `hits`, and this recorder's
`seen`/`matched`/`dropped`, stay. No schema keyword can express "this number must not have been reset",
so the whole of that half is held by
`watch_hits_epoch::the_aggregates_survive_every_boundary_while_the_epoch_relative_records_are_dropped`,
which is item 28's new positive row. Without it, an implementation that cleared **everything** at a
boundary passes every vector in this file and every other row in that one.
...
| | previous copy (`75b16d0`) | this copy (`c5638e6`) | delta |
|---|---|---|---|
| method fragments | 68 | **68** | unmoved — this CR adds no method |
| fragments declaring / closing `params` / declaring `result` | 68 / 68 / 68 | **68 / 68 / 68** | unmoved |
| event fragments | 4 | **4** | unmoved — `stopped`, `resumed`, `romReloaded`, `clicked` |
| `$defs` | 19 | **19** | unmoved — byte-identical; `hitsDropped` is declared inline on each fragment, not shared |
| `$schema` / `$id` / `title` / `description` / `anyMessage` / `handshake` | — | **byte-identical**, all six | unmoved |
| total leaf paths | 3011 | **3019** | +8 net: **9 added, 1 relocated, 0 values changed** |
| fragments whose JSON changed at all | — | **exactly two** | `methods["emulator/restore"]`, `events["emulator/romReloaded"]` |
| `restore.result` shape | bare `{"$ref": "#/$defs/replyFields"}` | **`allOf: [{$ref}]` + `required` + `properties`** | the relocation; the `$ref`'s value is unchanged and still applies |
| `restore.result.required` | *(absent)* | **`["hitsDropped"]`** | new — the fragment had no required key of its own |
| `romReloaded.params.required` | `["path"]` | **`["path","hitsDropped"]`** | +1, and the first `required` this file has ever seen move on an **event** |
| `romReloaded.params.properties` | `path`, `buildId` | **`path`, `buildId`, `hitsDropped`** | +1 |
| added leaves carrying validation force | — | **all 8** | `type: integer` + `minimum: 0` + membership of `required`, on both fragments |
| bytes | 355390 | **356644** | +1254 |
| **vectors** cases | 272 | **276** | **+4**, 0 removed, 0 changed; the prefix of 272 is identical |
| vectors `expect: "fail"` / `"pass"` | 162 / 110 | **164 / 112** | +2 / +2 |
| vectors: distinct methods named | 37 | **39** | **+2**: `emulator/restore` and `emulator/romReloaded` had **no** vectors before this |
| vectors carrying `group: "events"` | 8 | **10** | +2 |
| vectors `$comment` / `envelope` / `eventEnvelope` / `specExamples` | — | **byte-identical**, all four | unmoved |

The two failing vectors both fail on **absence**, which is the same assertion made twice because the key
is required in two places: an empty `restore` result, and a `romReloaded` params carrying `path` alone.
Neither can witness the obligation §11.39 spends its sharpest sentence on — that the event's count is the
**same number** as the reply's. Two documents each carrying a legal integer are two legal documents; the
equality lives across two messages, which is a place no per-fragment schema can look. That is
`watch_hits_epoch::the_rom_reloaded_event_carries_the_same_hits_dropped_as_the_reply_that_caused_it`,
which reads both messages off one connection and asserts them against each other and against the ring.

### What the PREVIOUS re-vendor adopted — §11.38 (CR-O): `hitsDropped` on `reload_rom` and `reset` (2026-09-05)

**Two required leaves, on two fragments, and five vectors — the first re-vendor in this file's recent run
where both artifacts' bytes moved.** §11.38 is **this lane's own CR**, raised from a consumer report:
aeon's first `emulator/watchpoint_hits` read after an `emulator/reload_rom` handed them hits stamped
frames **397 and 655**, recorded against a previous build and indistinguishable on the wire from the new
run's toggles. A hit is epoch-relative in three fields at once — `frame`, the cycle stamp, and a `pc`
resolved against whatever symbol table is loaded *now* — so a survivor is uninterpretable, not merely
stale. Adopted **as proposed**: both boundaries clear the ring and both report the count. The
epoch-counter alternative was declined on the record (`rom_generation` is waiting the day a consumer wants
reset-and-compare history; none does today). Reviewer named in the ruling: **aeon**, on the drive that
produced the stale read.

Figures **re-derived by parsing both copies**, never read from a commit message. Both were flattened to
leaf paths and compared **structurally**, not textually:

| | previous copy (`90aa1df`) | this copy (`75b16d0`) | delta |
|---|---|---|---|
| method fragments | 68 | **68** | unmoved — this CR adds no method |
| fragments declaring / closing `params` / declaring `result` | 68 / 68 / 68 | **68 / 68 / 68** | unmoved |
| `$defs` | 19 | **19** | unmoved — `hitsDropped` is declared inline on each fragment, not shared |
| leaf paths added / removed | — | **8 / 0** | 4 per fragment × 2: `hitsDropped`'s `type`/`minimum`/`description`, plus its entry appended to `result.required` |
| **leaf values changed** | — | **0** | not one pre-existing leaf moved — no stowaway this time, unlike the §11.37 re-vendor |
| fragments whose JSON changed at all | — | **exactly two** | `emulator/reload_rom`, `emulator/reset` |
| added leaves carrying validation force | — | **all 8** | `type: integer` + `minimum: 0` + membership of `required`; a server not serving the key now FAILS the fragment, which is the point |
| `reload_rom.result.required` | `["path","romBytes","symbolsDropped"]` | **`[…,"hitsDropped"]`** | +1 |
| `reset.result.required` | `["deferred"]` | **`["deferred","hitsDropped"]`** | +1 |
| bytes | 353750 | **355390** | +1640 |
| **vectors** cases | 267 | **272** | **+5**, 0 removed, 0 changed; the prefix of 267 is identical |
| vectors `expect: "fail"` / `"pass"` | 160 / 107 | **162 / 110** | +2 / +3 |
| vectors: distinct methods named | 35 | **37** | **+2**: `emulator/reload_rom` and `emulator/reset` had **no** vectors at all before this |

The two failing vectors are the load-bearing half and they fail for two different reasons: one omits
`hitsDropped` entirely (absence and `0` must not both mean "nothing was lost"), the other sends `-1`
(a count is never negative — that is what `minimum: 0` is for, and without the vector the keyword would be
unwitnessed).

### What the re-vendor BEFORE that adopted — §11.37 (CR-N): the `status` caveat's scope, description-only (2026-09-05)
...
### What the re-vendor THREE BACK adopted — §11.36 (CR-M): `emulator/lookup_equate`, 67 → 68 fragments (2026-09-05)
