...
use oracle_core::render::{cram_divergence_caveat, CandidateVerdict, Layer, LayerMask, PixelState};
...
impl Engine {
    ...
    fn pixel_attribution(&mut self, params: &Value) -> Result<Value, RpcError> {
        ...
        //
        // The verdict itself is `oracle_core::render::cram_divergence_caveat`, not a private rule here:
        // the player's click panel (`oracle-frontend`'s `pick::resolve`) answers the same question about
        // the same dot in-process, and D15 forbids it reaching this handler for the answer. One
        // implementation under both consumers, exactly as `sprite_tile_at` above; `pick.rs`'s `bus_parity`
        // is the assertion that says so if they ever drift.
