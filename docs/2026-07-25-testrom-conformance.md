...
| `m68k_opcode_sizes` (`m68k_opcode_sizes.bin`) | Per-opcode size/encoding sweep | Frame hash only — **not scraped**: at the pinned 120-frame budget the screen shows the ROM's font/pattern page, not a result page | `frame_hash=0x5436cda5786ea450` (re-pinned 2026-08-02 with the K1 fix; was `0xca81010c64f8e701` — 476 px moved across the `$0/4/8/C/E` opcode pages, the newly-trapping encodings). ⚑ **THIS PIN IS A DETERMINISM PIN, NOT A CONVERGENCE ONE** — see the 2026-09-19 note opposite | Unscraped by choice. Finding its result page (a longer budget or an input) is deferred. ⚑ **2026-09-19: BOTH HALVES OF THAT DEFERRAL ARE MEASURED FALSE, and the row's pin is of a moving picture.** Probed at this seat (`testrom_probe`, `PRESS=`): the decoded plane text is **identical at 120, 600 and 1800 frames** and unchanged under `start`, `a` or `c`, so neither a longer budget nor an input reaches a result page — the screen holds the ROM's ASCII character ramp throughout. **But the frame hash differs at every budget**: `0x5436cda5786ea450` (120) → `0x102fe6ffdd51e11c` (300) → `0x5330009202fa2287` (600) → `0xb1e54eed02744f27` (1800). So this row is pinned to a picture that is still in motion at thirty seconds of emulated time, stable only because the run is deterministic. **That is a determinism pin and the row now says so** — the defect was never the motion, it was a pin not saying which property it rests on (`F-TIMED-PIN-UNSETTLED`). ⚑ **The cheap surface lied**: the plane text agreed at every budget while the full hash differed at all four, and the plane text is what a reader checks when asking "has it finished?". Whether the ROM has a verdict page at all, and whether our machine is failing to complete it, is row `TESTROM-OPCODE-SIZES-NEVER-SETTLES` — possibly a core finding, not a harness one. |
