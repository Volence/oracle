...
| `m68k_illegal` (`itest`) | Illegal / privileged / unimplemented encodings must trap to the right vector | **Yes** — no text; verdict is the backdrop word `CRAM[0..2]`: blue `$0E00` while running, `$00E0` green = pass, `$000E` red = fail. **Stop condition** (2026-08-14): runs until the ROM writes the green-or-red verdict to `CRAM[0]`, bounded at 20 frames and asserting the stop actually fired. Measured: `$0000` then `$0E00` on frame 0, `$00E0` on frame **9** | **PASS** — backdrop `$00E0` (green) since the 2026-08-02 K1 fix (was FAIL `$000E`) | **K1 — RESOLVED & FIXED.** See below. |
...
| `vdp_port_access` (`VDPFIFOTesting`) | 16 VDP port-access tests over two pages: FIFO size/behaviour, DMA-via-FIFO, byteswapping, partial CP writes, register-write masking, read-target switching, FIFO wait states | **Yes** — scrapes the on-screen `Results: ( P/ F/ T)` line; page 1 auto-runs (settles ~frame 42), `Start` advances to page 2 (~480 more frames) | **page 1 = 9 pass / 0 fail / 9 (complete)**; **pages 1+2 cumulative = 16 pass / 0 fail / 16 — this ROM is COMPLETE** (was 9/7/16 — slice A2 flipped T13 and T10, slice A3a flipped T3, slice A3b flipped T4, slice A4 flipped T6, slice T12 flipped T12, slices T16/S1 + T16/S2 flipped T16; Test 16's byte matrix had already improved 54→18 red cells with the A1 live FIFO flags — see notes below) | The harness remains non-gating on this ROM (the CHARTER line stands, and `conformance_roms.rs`'s header still states it); what changed on 2026-08-03 is that the owner asked for this ROM's rows to be worked, so they are now being fixed slice-by-slice (A1 = live FIFO EMPTY/FULL flags; A2 = control-port / code-register edges; A3a = DMA payload words through the FIFO ring; A3b = the DMA-fill trigger write + `address ^ 1` fill bytes; A4 = the 8-bit VRAM read target, all 2026-08-03). Per-test today — page 1: **T1-T9 all pass**; page 2: **T10-T16 all pass**. **A4 flipped T6 "8-bit VRAM Read target 01100"** — CD `%001100` returns `vram[address ^ 1]` in the low byte and the next-available FIFO entry's high byte in the high byte (see the A4 addendum below); it is currency-neutral and changed no existing test. **A3a flipped T3 "DMA Transfer using FIFO"** and **A3b flipped T4 "DMA Fill FIFO Usage"** (see the A3a/A3b notes below). A3b was expected to move `export_state_v1::GOLDEN_HASH` and **does not** — the design note's §4.1 mis-identified the golden fixture (it is `testrom::build`, which never touches the VDP; the DMA-filling ROM is `testrom::build_pad_poll`, used only by the io/watchpoint tests). Every currency gate is byte-identical across A3b. **A2 flipped T13 "Register Writes and Code Reg" and T10 "Partial CP Writes"** (see the A2 note below), and left **T12 "Register Write Mode4 Mask"** as a named residual, **which slice T12 has now FIXED (2026-08-03)**: in Mode 4 (reg 1 bit 2 = M5 clear) only the eleven SMS registers 0–10 are writable, and writes above 10 are discarded (one line in `Vdp::write_register`). **CORRECTION — the reason this was held was false.** The claim that the fix "moves `export_state_v1::GOLDEN_HASH` and the `golden_frames` scenes" rested on the *same* fixture mis-identification as A3b's: it cited `testrom.rs`'s `reg 1 = $8150`, which lives in `testrom::build_pad_poll` (used only by `io_controllers`, `watchpoints` and the `pad_probe` example — no frozen currency), whereas `export_state_v1` loads `testrom::build`, which drives no VDP port at all. Re-measured: **no frozen constant moves.** 66 tests did go red, every one the same fixture defect — fixtures that write `reg 1 = $40` (M5 clear) and then program registers 11+, i.e. they intend Mode 5 but never declare it, a machine state that cannot exist on hardware. Declaring M5 in them (test-only, 8 sites) brought all frozen hashes back **byte-identical**. See `docs/2026-08-03-decision2-premise-recheck.md` and the T12 addendum below. The spec test `vdp::tests::mode4_ignores_register_writes_above_ten` is no longer `#[ignore]`d. **Evidence caveat:** the `> 10` boundary is extrapolated from a hedged source — follow-up **F-M4REGS**. **T16 "FIFO Wait States" PASSES — all 80/80 verdict bytes green (was 26/80, then 62/80, then 72/80).** Two independent causes, fixed in two slices, and *neither* was the "Phase 3 per-line DMA cost" deferral this residual had been filed under. **T16/S1** gave the FIFO drain the *positions* of the external access slots within an active display line instead of a uniform per-line rate, flipping groups 2/3/5/6/8 [62→72]. **T16/S2** made a finished 68k→VDP DMA leave words **pending** in the FIFO — the DMA unit's job ends when the last word is pushed into the FIFO, not when it reaches VRAM — flipping groups 9/10 [72→**80**]; that reverses A3a's deliberate ring-store-only choice and answers design question Q1 of `docs/2026-08-03-a3-dma-fifo-design.md`, which A3a had explicitly deferred to here. (**A3a landed DMA-through-FIFO ring *contents* and left T16 unmoved at 62/80** — which is exactly what identified groups 9/10 as needing *occupancy* rather than contents.) See the S1 and S2 addenda below. **2026-09-12 (VDP-PORT-ACCESS-FULL-ROM):** "COMPLETE" was true of the two pages the scraper read, not of the ROM. The ROM runs **122 tests over 22 pages** and prints **76 / 46 / 122**. The row now also reads the last page, and `vdp_port_access_full_rom_verdicts` pins the 46 failures by number and title. They come from six causes, five behaviour (A1 VSRAM address decode, A2 128 KB DMA source wrap, A3 fill/copy advancing the source registers, A4 busy set by the fill command, A5 a fill with no write target) and one unmodelled (M1, a fill running over time). The same doc corrects two claims below: F-COPYXOR's residual (the copy matrix's first eight words are A1, not a read path) and F-FILLTGT's "the ROM does not cover the case" (test 34 does). See `docs/2026-09-12-vdp-port-access-full-rom.md`. **2026-09-12 (DMA-SRC-ADVANCE, cause A3):** a fill and a copy now advance source registers 21/22 by one per step (so by their length in bytes), with 21 carrying into 22 and nothing carrying into register 23 (`Vdp::advance_dma_source_low16`). Tests 28 "DMA Fill Source Reg Update" and 29 "DMA Copy Source Reg Update" flip, as the write-up's EXP=8 row predicted. All 22 pages go **76/46/122 → 78/44/122**, and pages 1 and 2 are unchanged. `PORT_ACCESS_FAILING` was re-derived from the run: it is the 46 minus 28 and 29, with no count moved. The other 16 scorecard rows and `oracle-core`'s four frozen-golden binaries are byte-identical. See A3 in the same doc. **2026-09-12 (VSRAM-DECODE, cause A1 fixed):** the VSRAM address is now 7 bits (`Vdp::vsram_byte`: it wraps at `$80`, writes to `$50-$7F` are discarded) and a read of `$50-$7F` returns a new VSRAM read latch, which only the committed render feeds (`render_scanline` and `advance_scanline` through one helper). **All 22 pages 76/46/122 → 112/10/122**, failing exactly 20 27 28 29 31 32 33 34 36 38. Tests 23, the eight VSRAM fills (74-95) and the copy matrix (96-122) flip to pass, test 13 stays green, and test 20 goes from 8/12 to 6/12 words off (the rest is A2). Pages 1 and 2 are unchanged at 9/0/9 and 16/0/16. See the dated "Fixed" note under A1 in that doc. **2026-09-12 (DMA-SRC-128K, cause A2 fixed):** a 68k-to-VDP transfer's source is confined to one 128 KB page. Source registers 22:21 are a 16-bit word counter and register 23 never takes their carry, so a transfer that runs off the top of the page resumes at its bottom (`MegaDriveBus::run_mem_dma` walks `page | off`; `Vdp::dma_complete` now shares `Vdp::advance_dma_source_low16` with fill and copy and stops writing register 23 at all). **All 22 pages 114/8/122 → 116/6/122**, failing exactly 31 32 33 34 36 38. Test 27 "DMA Transfer Source Reg Update" flips on A2 alone; test 20 "DMA Transfer Source Wrapping" is a JOINT flip with A1 (8/12 words off → 6/12 after A1 → 0/12 now), so it took both. Pages 1 and 2 are unchanged at 9/0/9 and 16/0/16. `PORT_ACCESS_FAILING` and the row's tally were re-derived from the run. The other 16 scorecard rows and `oracle-core`'s four frozen-golden binaries (`determinism_gate`, `export_state_v1`, `golden_frames`, `scanline_goldens`) are byte-identical — expected, because the old register-23 write put back the value it already held for any transfer that stayed inside its page. A mutation found that the 16-bit WIDTH of the 22:21 counter had no guard at all, A3's included; `bus::tests::a_mem_dma_advance_keeps_all_16_bits_of_registers_22_21` now pins it for both. See the dated "Fixed" note under A2 in that doc. **2026-09-12 (FILL-BUSY-ARM, cause A4 fixed):** the DMA-busy status bit now reads set from a fill's **control** write, not only from its data-port trigger — `Vdp::dma_busy` is "a fill is armed (CD5 latched + register 23 naming Fill) OR the transfer window is open", derived from existing state rather than latched in a new field. Eke, *VDP Internals* p.4: "on DMA Fill, busy flag is actually immediately (?) set after the CTRL port write". Tests 36 "DMA Busy Flag DMA Fill" and 38 "DMA Busy Flag DMA Toggle Fill" flip, as the write-up's EXP=16 row predicted. **All 22 pages 114/8/122 → 116/6/122**, failing exactly 20 27 31 32 33 34; page 1 stays 9/0/9 and pages 1+2 stay 16/0/16. `PORT_ACCESS_FAILING` was re-derived from the run. The other 16 scorecard rows are byte-identical.  See the dated "Fixed" note under A4 in that doc. **2026-09-12 (FILL-TGT, cause A5 fixed; follow-up F-FILLTGT RETIRED):** `Vdp::run_fill`'s body now shares `code_names_a_write_target` with the ordinary data-port write and the fill-trigger write, so a fill armed on a code whose low nibble names no write target **runs and writes nowhere** — it still walks its address, counts its length to 0, advances source registers 21/22 and opens its busy window. The evidence is test 34 "DMA Fill Control Port Writes" group 3 (ROM `$45B2`, code `$4898..$4948`), which retires F-FILLTGT's "the ROM does not cover the case". That test flips, as the write-up's EXP=32 row predicted. **All 22 pages 116/6/122 → 117/5/122**, failing exactly 20 27 31 32 33 — the two A2 tests and M1's three. Page 1 stays 9/0/9 and pages 1+2 stay 16/0/16; tests 4, 28, 29 and 72-95 are unmoved, which is what proves this is a suppressed write and not an early return. `PORT_ACCESS_FAILING` was re-derived from the run. The other 16 scorecard rows are byte-identical.  See the dated "Fixed" note under A5 in that doc. **2026-09-12, A2 + A4 + A5 merged**, re-derived from one run on the merged tree: **all 22 pages 119/3/122, failing exactly 31 32 33** — M1 (FILL-OVER-TIME) alone, the size-L design item that is deliberately out of scope. Page 1 stays 9/0/9 and pages 1+2 stay 16/0/16. Five of the six causes this ROM revealed (A1-A5) are now fixed; the remaining three failures are one unmodelled mechanism, not five. **2026-09-15 (M1 FILL-RUN, the sixth cause; parcel P1 of `docs/2026-09-14-m1-fill-over-time-design.md`, under hub rulings R1-R4): THIS ROM NOW PASSES ALL 122 TESTS.** A DMA fill is no longer something that happens inside its trigger write: it is a process the VDP advances lazily on `fifo_slot_clock`, the one external access-slot clock it shares with the FIFO drain — pending FIFO entries first, then one fill step per slot — caught up at every VDP port access, before every active line renders, and at the end of every run. Each step pulls its write target and its data out of the **FIFO ring** (the newest entry's high byte for VRAM, the next-available entry for CRAM/VSRAM, Nemesis's documented latch bug) rather than from a copy of the trigger word, and a data-port write made while a fill runs is an ordinary FIFO write that consumes **no** length count. Progress lives entirely in state the machine already had — registers 19/20 counting down (CD5 clears at 0, which is also RD2's "length 0 = 65,536"), the address register, registers 21/22, the FIFO ring and its clock — plus **one** new bit, a trailing `DmaRequest::FillRunning` variant. Tests 31, 32 and 33 "DP Writes During DMA Fill VRAM/CRAM/VSRAM" flip, each on the exact tail §1.3 of the design derived by hand from the ROM's code before the model existed. **All 22 pages 119/3/122 → 122/0/122**, and `PORT_ACCESS_FAILING` is now **empty**; pages 1 and 2 stay 9/0/9 and 16/0/16. The other 16 scorecard rows are byte-identical, as are the scanline goldens, `golden_frames`, `determinism_gate` and `export_state_v1`, and the save-state layout fingerprint does not move (`dff350afa2eb3e1d` before and after, measured — old saves, including mid-fill ones, load in the new build and evolve to the old build's own hashes). What DID move, and is recorded rather than smoothed over: a fill's elapsed time (its busy bit now clears when the last step runs, not when a flat-rate window expires), the mclk stamp on a fill's VDP writes (per step, hub ruling R2 — decision C-6 stands for 68k-to-VDP DMA), and three synthetic fixtures that started a 64 KiB fill and then wrote the VDP without waiting, which now clear the display and busy-poll first (hub ruling R4) as the hardware rule and every real ROM require. |
| `vdp_sprite_masking` (`SpriteMaskingTestRom`) | 9 sprite masking / per-line / per-frame / dot-overflow tests | **Yes** — verdict is a 32×8 glyph at the right edge, classified by **rendered-pixel hash** (its nametable cells are identical for the tick and cross cases, so only the framebuffer discriminates). Since 2026-09-18 the pixels are the **live** ones — a `ScanlineCapture` of the last completed frame — and the post-hoc re-render is classified alongside and reported in the row as the control (see the `[post-hoc path …]` bracket). **Stop condition** (2026-08-14): runs until the VDP port block has been idle for 8 consecutive frames, bounded at 300 and asserting the stop fired — measured over the full 300, every VDP access is on frames 0-7 and there is none on 8-299, so it stops at 15; the captured frame is index 14 and guards require it to be one whole frame drawn strictly after the ROM's last VDP write. **Since 2026-09-18 (TESTROM-H40-HALF) the ROM is scraped in BOTH its screen modes from one boot** — H32 to idle, a `C` press, H40 to idle (stop at frame 30, captured frame 29) — and the mode each half is in is PROVED off the captured picture before a glyph is read: `ProvenScreen::establish` takes `CompletedFrame::width()` (the width the frame ENDED on, never a post-hoc re-query of the chip, which after a switch answers for the next frame), and `sprite_masking_classify` cannot be called without the value it returns | `H32: 1=TICK/TICK 2=TICK/TICK 3=TICK/TICK 4=PASS 5=PASS 6=PASS 7=PASS 8=PASS 9=TICK/TICK [post-hoc path differs: 6=FAIL] \| H40: 1=TICK/TICK 2=TICK/TICK 3=TICK/TICK 4=PASS 5=PASS 6=PASS 7=PASS 8=PASS 9=TICK/TICK [post-hoc path differs: 6=FAIL]` — **0 failures, in BOTH of the ROM's screen modes** (H40 half added 2026-09-18). This ROM passes all nine of its own tests. The bracket is not a caveat on that, it is the **control arm**: the same nine glyphs classified through the post-hoc re-render, which still reads test 6 as `FAIL` for the reason **F-POSTHOC-STALE-CARRY** named and which is now CLOSED (below). | **2026-09-18 (TESTROM-H40-HALF): the row gains an `H40:` half; Q1 CLOSED; no emulator behaviour changed.** This ROM runs its nine tests in two screen modes and the harness had only ever measured the power-on one — which matters because the per-line sprite limit (20 vs 16), dots per line (320 vs 256) and sprites per frame (80 vs 64) are all per-mode, and tests 1, 2, 3 and 9 are tests OF them. Nine H40 verdicts read for the first time; **all nine pass and all nine classify against the four already-pinned glyph hashes with no re-derivation** (the L3 trap avoided a third time). The live-vs-post-hoc split on test 6 **reproduces identically in H40**, so it is not an H32 accident, and the control arm now requires the divergence in BOTH halves. Coordinates derived in H40, not scaled: the verdict rectangle is x 216..248 in both modes because the ROM writes its verdict cells at plane columns 27-30 regardless of mode instead of right-aligning them — confirmed off the H40 framebuffer, where all nine glyphs' ink falls in x 216..=246 and x 248..319 is blank on all nine rows. The mode LABEL is measured too; the old row's `"H32:"` was a string literal that could not be wrong. The retired `pixels().len() == width * ACTIVE_LINES` assert is replaced by `refuse_a_ragged_frame`, which can see a frame torn ACROSS a mode switch (`CompletedFrame::pixels` would otherwise pad one into a confident rectangle) and which is tested directly because **this ROM structurally cannot produce that tear** — the R12 write is itself a VDP write, so the torn frame is always nine or more frames older than the retained one. Of the 17 rows this is again the only one that moved, and its H32 segment is character-for-character the old row (measured base-vs-branch, the other 16 rows identical by SHA-256). See `docs/2026-09-18-h40-half.md`. Previous note — **2026-09-18 (POSTHOC-CARRY): `6=FAIL` → `6=PASS`; F-POSTHOC-STALE-CARRY CLOSED; no emulator behaviour changed.** The glyphs are now read from the picture the VDP emitted rather than from a re-render of the settled machine, because `Vdp::render_line` re-seeds R10 masking from the *end-of-frame* `sprite_dot_overflow_carry` on every line while `Vdp::render_scanline` advances it — and test 6 is this ROM's test OF that carry. Measured: the settled carry is `false`, the live walk commits `true` on line 87 and holds it to line 94, so lines 88-95 × x 216..247 (8 of 224 lines, and exactly the rectangle read for test 6) are drawn with the mask armed live and unarmed post-hoc. Both glyph-constant blockers settled by measurement — **no constant was re-derived** (test 6's live hash *is* the already-pinned `PASS` literal), and the frame read is the last one the run **completed**. The post-hoc reading is kept in the row so a future divergence in either direction moves the scorecard. Previous note — **2026-09-16 (SPRITE-MID-CUT): test 3 flips `TICK/CROSS` → `TICK/TICK` and this row's one real failure is closed.** The per-line sprite-pixel budget now cuts **inside** the sprite that straddles it (`Vdp::sprite_line`), so MAX SPRITE DOTS – COMPLEX — the sub-case this row credited to ledger row **P1** — passes; P1 is closed except for the interim *order* of the surviving dots (see it). Verdict string re-derived from the run, not edited by hand. **No glyph constant was re-derived**: the new glyph hash matched the already-pinned `TICK_TICK`, so the L3 / F-POSTHOC-STALE-CARRY trap (the four constants are themselves pinned from post-hoc pixels) was never entered. `6=FAIL` was deliberately **not** touched then; it is what POSTHOC-CARRY settled. Of the 17 scorecard rows this is the only one that moved, in both parcels. Open question **Q1** on the H32/H40 toggle was CLOSED by the 2026-09-18 half above. |
| `color_1536` (`TEST1536.BIN`) | 1536-colour trick — CRAM rewritten mid-scanline | Frame hash only — **per-scanline capture** (the only row that uses it) | `frame_hash=0x87ecf46f3cd54fda` (per-scanline capture, **sub-line CRAM**; re-pinned 2026-09-08. Was `0x9ae4acc58d2a382d` from 2026-08-19; `0x917371f07409cb25` per-scanline/line-atomic from 2026-08-03; before that `0x96b9c93c4f3dd325` end-of-frame) | **Limitation L1 — NARROWED TWICE**: 2026-08-03 gave the row the picture the ROM draws *between* lines; 2026-08-19 (`F-SCANLINE-SUBLINE` slice 4) gives it the picture *within* a line. Measured mechanism: 515 value-changing CRAM writes per hashed frame inside the active-display window, to indices 4–7, over active lines 48–221 — all four sampled by the picture. Still a regression pin, not a verdict (the ROM prints none). |
...
| `window_distortion` (`Window distortion bug.BIN`) | The hardware window-plane distortion bug (left window + fine h-scroll) | Frame hash only | `frame_hash=0x5102219d295b4e2c` ⚑ **determinism pin, NOT convergence: this ROM animates, so it has no settled frame** — measured 2026-09-19 at 600 frames as `0x0650901b01e7c61e` (`F-TIMED-PIN-UNSETTLED`) | Ledger row **P5** (R9 window-bug sub-tile alignment) governs the exact reused-tile offset. |
| `vdp_test_register` (`DisableRegTestROM.bin`) | The undocumented VDP test register (`$1F`) display-disable bits | Frame hash only | `frame_hash=0x4a49cfea306e928b` ⚑ **determinism pin, NOT convergence: this ROM animates, so it has no settled frame** — measured 2026-09-19 at 600 frames as `0x705f6a27f2181823` (`F-TIMED-PIN-UNSETTLED`) | We do not model the test register; the hash is a "did the picture move" pin, not a verdict. |
...
| `m68k_opcode_sizes` (`m68k_opcode_sizes.bin`) | Per-opcode size sweep: the length of **every one of the 65536 opcode words**, measured by running each one at `$FFFF86` in front of 32 words of `$F000` padding and turning the trap's stacked PC into a byte count (`move.w $4(a7),d1` / `subi.w #$FF86,d1` at ROM `$42C`; vectors 4/10/11 all point there, so an illegal opcode yields 0 and a valid one yields its size). Plotted as **one pixel per opcode, colour = length in words, colour 0 = illegal**, then checksummed by the ROM against its own baked constants | **Yes, since 2026-09-18** (`TESTROM-OPCODE-SIZES-NEVER-SETTLES`) — and it is the corpus's strongest verdict: the ROM CRC-32s its own 32768-byte pixel map, and an 8192-byte bitmap of which opcodes it *measured* versus *asserted from a table* (ROM `$472` refuses to execute `Bcc`/`BSR`/`DBcc` and friends), against two constants baked into the image (ROM `$56C`, `$57E`), painting **colour 6 for match and colour 9 for mismatch** at VRAM `$8000` (ROM `$682`/`$68C`; palette 6 = `$00E0` green, 9 = `$000E` red). **The verdict is a COLOUR, not a word** — which is why a printable-ASCII sweep of the image finds no `PASS`/`FAIL`/`error` anywhere. `OpcodeSizesRun::establish` is the only constructor and the only route to `verdict()`; it **waits** for the ROM to park in its own pad wait (`$5FA-$62A`, a release edge on any button, unreachable without input) rather than timing it, checks the band's shape (32 identical bytes per tile, zero separators, a uniform verdict nibble), and — the load-bearing condition — requires **our own CRC-32 of the live machine to equal the CRC the band displays**, so a read off the wrong tiles or of a map the ROM never checksummed is not representable. It does NOT require the CRCs to match the ROM's expectation: that is the reading, and a failing emulator has to stay representable. Established at 700 frames | `2/2 crc32 sizemap=0x5c6da501 classmap=0x20ac2324` — **both halves are real PASSes**, and both are independently recomputed over our own machine (`VRAM_CRC=0,8000` -> `0x5c6da501`; `RAM_CRC=8000,2000` -> `0x20ac2324`), so this is not "the ROM printed green": the 65536-entry instruction-length table our decoder produced is byte-exact against a constant computed on hardware in 2017. (was `VISUAL-BASELINE frame_hash=0x5436cda5786ea450`; before the K1 fix `0xca81010c64f8e701`) | **2026-09-18 (`TESTROM-OPCODE-SIZES-NEVER-SETTLES`): every reason this row has ever carried was wrong, the 2026-09-19 amendment included.** (a) "At the pinned 120-frame budget the screen shows the ROM's font/pattern page" — **there is no font page.** The nametable is an **identity map** (ROM `$3C6-$3D8`: 64 ascending cells per row then `subi.w #$18,d3`, i.e. `cell = row*40 + col`), written once at boot and never touched; its tile indices `$20-$7E` decode as printable ASCII in a text-grid dump, and that is the whole of the "ASCII character ramp". **That channel is constant by construction and could never have carried a verdict, at any budget** — the sharper form of "the cheap surface lied": it did not lie by being coarse, it lied by being unable to vary. (b) "This ROM animates, so it has no settled frame" (`F-TIMED-PIN-UNSETTLED`) — **FALSE. It settles.** Every budget from 685 to 3600 frames gives `frame_hash=0xb1e54eed02744f27`; 400-675 all give `0x5330009202fa2287`. The four samples that read as perpetual motion (120/300/600/1800) straddled the two transitions, and at 1800 frames the picture had been static for ~18.7 s. This pin was unsettled because the budget was too **short**, not because the picture never settles. (c) "Finding its result page (a longer budget or an input) is deferred" — the budget was the entire blocker, and it is now quantified: at frame 120 the ROM's own opcode counter at `$FFFF86` reads **`$46AF`**, i.e. **27.8% through the sweep**, with tiles `$200`/`$300`/`$380` (the `$8xxx`/`$Cxxx`/`$Exxx` pages) measurably all-zero. No input is needed for the verdict at all — the pad wait comes *after* it, which is also why the 2026-09-19 probe's presses at frame 60 could not have done anything (`$FFA000` is not seeded until ROM `$5E4`, ~frame 680); pressed at frame 700, every button moves the picture. ⚑ **This also makes this row's own K1 note impossible as stated**: "476 px moved across the `$0/4/8/C/E` opcode pages" cannot describe a 120-frame pin, because three of those five pages are not plotted in it, and `scrape_visual`'s budget has been 120 since the suite's first commit (`7b46ae2`, confirmed with `git log -S`). What the 476 figure measured is not recoverable without rebuilding the pre-K1 core, so it is recorded as an inconsistency rather than corrected. The K1 fix **is** in the settled picture, at tile `$200` row 1 (`OR.b An,D0` = `$8008-$800F`, colour 0 = illegal). The pixel layout was cross-checked against the 68000 rather than against us: tile `$200` (`OR.b <ea>,D0`) reads `1x8 / 0x8 / 1x8 / 1x8 / 1x8 / 2x8 / 2x8 / 2,3,2,2,2,0,0,0` = Dn 1 word, **An illegal**, `(An)`/`(An)+`/`-(An)` 1, `d16(An)`/`d8(An,Xn)` 2, `(xxx).w` 2, `(xxx).l` 3, `d16(PC)`/`d8(PC,Xn)`/`#imm` 2, modes 7/5-7/7 illegal. Measured surprise worth its own line: `moveq #$FF,d0` at ROM `$656` **sign-extends**, so the digest seed is `$FFFFFFFF`; read as the literal `$000000FF` it yields `0x3e13d6a8` and would have reported our CPU as getting 65536 opcode lengths wrong. Six red-first mutations, each with the guard it fired named — including one that fired the **wrong** guard and was re-cut, and one whose red note named the wrong cause and was rewritten. Two conditions are recorded as not independently red-provable without a `src/` change. No file under `crates/oracle-core/src/` was touched and the other 16 rows are byte-identical. See `docs/2026-09-18-opcode-sizes-diagnosis.md`. |
...
⚑ **2026-09-18: the span in that parenthesis cannot be right**, and it is left standing only because what it
actually measured is not recoverable from here. The pinned frame is 120 frames, and at frame 120 this ROM's
own opcode counter reads `$46AF` — so the `$8xxx`, `$Cxxx` and `$Exxx` pages are not plotted in it at all
(measured: tiles `$200`/`$300`/`$380` are all-zero at 120 frames and populated at 800). The K1 fix *is*
visible in the ROM's completed picture — tile `$200` row 1 is `OR.b An,D0`, colour 0 = illegal — and as of
2026-09-18 this row reads that completed picture's own CRC-32 verdict instead of a partial hash. See
`docs/2026-09-18-opcode-sizes-diagnosis.md`.
...
* **Per-scanline, sub-line CRAM, C2-corrected stall billing** (`0x87ecf46f3cd54fda`, 2026-09-08): the
  picture is unchanged in kind; what moved is *when* the ROM's mid-scanline CRAM writes land. `data_write_at`
  billed a full-FIFO /DTACK stall from the 68000 instruction's frozen `now` rather than from the drain
  instant the CPU had already been held to, so a second stalling data-port write inside one instruction was
  charged the first one's hold a second time. Measured over the 120 hashed frames: **11 full-FIFO data-port
  writes, 5 of them same-instruction repeats, 1,239 mclk (~177 CPU cycles) of already-billed hold
  re-charged.** Correcting it changes those instructions' costs and therefore `now_mclk` downstream, and
  slice 4 below resolves each CRAM landing to a pixel, so the frame hash is sensitive to it. Re-pinned in
  `scanline_goldens.rs` and `conformance_roms.rs` together — both re-measured, both moved to this same
  value, cross-check preserved. **The only row in the corpus that moved**; every pass/fail verdict held.
...
**2026-09-16 update to that correction.** The one failure P1 *did* own is now fixed: the mid-sprite
pixel-budget cut is implemented (`Vdp::sprite_line`), test 3 reads `TICK/TICK`, and the row above is amended
with the re-derived verdict string. **The correction's conclusion survives its own subject** — the row now
carries exactly one failure and it is test 6, the artefact. The glyph constants were **not** re-derived (the
new test-3 hash matched the already-pinned `TICK_TICK`), so F-POSTHOC-STALE-CARRY is untouched, still open,
and still the reason `6=FAIL` stands.

**2026-09-18 — closed (parcel `POSTHOC-CARRY`).** The correction is now landed rather than recorded: the
scraper reads the live picture, the row says `6=PASS`, and `F-POSTHOC-STALE-CARRY` is closed below. The
premise was **re-derived from the tree** rather than taken from the 2026-08-15 note, and the derivation is
worth keeping because it names the one line that causes it. `Vdp::sprite_line` seeds its R10 x=0 masking
with `seen_nonzero = self.sprite_dot_overflow_carry()` and never writes the carry back; the write lives in
`Vdp::commit_scanline_sprites`, which only the *stateful* renders (`render_scanline`, `advance_scanline`)
call. A pure `render_line` therefore seeds **every** line from the carry as it stands after the run.
Measured on this ROM at its idle stop: that settled carry is `false`, while the live walk commits `true` on
line **87** and holds it through line 94 — line 95 is drawn with the seed still `true` and commits `false`,
because the mask suppressed the sprites and there was no overflow left to carry. So lines **88-95**, x
216..247, are drawn with the mask armed live and unarmed post-hoc: 8 differing lines out of 224, and
exactly the 32×8 rectangle the scraper reads for test 6. Independent cross-check: replaying the *stateful*
path over the settled machine in line order reproduces the live capture hash exactly
(`0x3f1249dcdab2e0ed`, the value `scanline_goldens.rs` independently pins for this ROM); the pure path does
not.

**What it cost to move, which is the part a future re-pathing should read.** Both of the blockers the
follow-up named had to be settled, and both were settled by measurement:

* *(a) the glyph constants.* All four survive with **no re-derivation**. Eight of the nine glyphs are
  bit-identical through both substrates, and test 6's live hash is `0x66094bba88cb93ed` — which **is** the
  `PASS` literal, pinned from post-hoc pixels in the first place. The scope of "every ROM and glyph" is
  smaller than it sounds: `block_hash` has exactly one caller, so it is these nine glyphs; the other 16
  rows scrape the nametable (`text_rows`), CRAM (`m68k_illegal`) or a whole-frame hash, none of which the
  substrate touches. Worth knowing: `TICK_CROSS` now classifies **nothing** — test 3's cross was the only
  one in the corpus and it flipped to a tick in 2026-09-16 — so it is an unexercised classifier, kept
  deliberately (see L3).
* *(b) which frame.* The last frame the run **completed** before the idle stop (index 14; the stop fires
  mid-frame 15). The 2026-08-14 idle-stop conversion could say "`block_hash` re-renders from live VDP
  state, so stopping mid-frame is irrelevant to it" — true of a re-render, **false of a capture**. That
  sentence is replaced by loud guards in `sprite_masking_half` (called `sprite_masking_glyphs` until
  2026-09-18): exactly one complete frame of active lines, no line of it a different width from the frame
  itself, at least one frame completed at all, and the captured frame **strictly later** than the last
  frame in which the ROM touched the VDP. Attaching the capture is machine-neutral — measured, both
  arms stop at the identical record (frame 15, pc `$316`, mclk 13,440,630) with byte-identical post-hoc
  frame hashes, which is `render.rs`'s `the_cheap_scanline_advance_leaves_the_same_machine` holding in the
  field.

**The control arm, which is why this parcel is worth more than the row flip.** A `FAIL` glyph looks the
same whether it is a ROM's verdict or our own instrument's artefact — that is the whole reason this stood
unresolved for a month. So the row now carries **both** readings: the live verdicts, then the complete list
of glyphs the re-render disagrees about, stated positively in both directions (`[post-hoc path agrees]`
when there is nothing to report, so an absence can never be read as an unmeasured value). Every direction
of drift is a scorecard diff — the live path collapsing back to post-hoc, a *new* disagreement on any
glyph, or the post-hoc path being "fixed" until it agrees, which was the hazard the old row comment could
only warn about in prose. Non-vacuity is `the_glyph_scrape_reads_the_live_render_path`, which requires the
two substrates to still be distinguishable here, requires the row to be oriented
live-in-body/post-hoc-in-bracket (built from labels it measures itself, never read back from `BASELINE`),
and refuses to let an `UNKNOWN-GLYPH` on either side pass as an agreement.

Red-first, each mutation shown applied on disk before the run (`conformance_roms`: 6 passed / 0 failed
green; every mutation below 4 passed / **2 failed**, the scorecard row and the control arm together):

| mutation | what it models | row under mutation |
|---|---|---|
| `block_hash_live` → `block_hash` | the live classifier re-pathed back to the re-render | `6=FAIL … [post-hoc path agrees]` |
| `Vdp::sprite_line`: `seen_nonzero = self.sprite_dot_overflow_carry()` → `false` | the **emulator's** live carry seeding broken | `6=FAIL … [post-hoc path agrees]` |
| post-hoc classifier → the capture | the post-hoc path "fixed" until it agrees (the old warning's trap) | `6=PASS … [post-hoc path agrees]` |
| `Retain::LastFrame` → `Retain::First` | a torn/short capture | guard: 256 pixels, not 256×224 |
| `VdpIdle::QUIET_FRAMES` 8 → 1 | scraping the frame the ROM was still drawing in | guard: captured frame 0 is not later than last-busy frame 0 |

*(2026-09-18, TESTROM-H40-HALF: the fourth row's guard was replaced when the scrape moved to
`CompletedFrame`. `Retain::First` now fires the **completed-frame** refusal instead — "completed no whole
frame of 224 active lines … logged 2912 lines holding 320 pixels" — because under that policy the line log
no longer sums to the pixels held. Still loud, still correct, but it is a proof of a different guard, so it
is recorded rather than carried over. The mutation table for the H40 half is in
`docs/2026-09-18-h40-half.md`.)*
...
* **F-POSTHOC-STALE-CARRY — the verdict-glyph scraper reads the wrong render path.** (Registered 2026-08-15 by
  ...
  (**2026-09-16:** the last clause is now moot from the other side — the mid-sprite cut parcel fixed the one
  failure P1 owned, so the row claims one failure and it is test 6. This follow-up is unchanged otherwise, and
  the parcel deliberately did **not** re-derive any glyph constant: the flipped test-3 hash matched the
  already-pinned `TICK_TICK`, so (a) below was never put to the test.)
  ...
  **CLOSED 2026-09-18 (parcel `POSTHOC-CARRY`) — the premise held, both blockers settled by measurement, and
  the row is `6=PASS`.** No emulator behaviour changed; nothing under `crates/oracle-core/src/` was touched.
  Full evidence and the red-first table are in the L1c section above. In brief: **(a)** all four constants
  survive with no re-derivation (test 6's *live* hash is the already-pinned `PASS` literal), and `block_hash`
  has exactly one caller, so "every ROM and glyph" means these nine; **(b)** the frame is the last one the run
  **completed** before the idle stop, and the old "stopping mid-frame is irrelevant" convention — true of a
  re-render, false of a capture — is replaced by three loud guards. The proposed amendment's third clause
  ("the P1 ledger row corrected to claim one failure instead of two") needed **no action**: 2026-09-16 had
  already rewritten that row and it makes no claim about test 6. The warning this bullet ends on is now
  structural rather than prose — "fixing" the emulator until the post-hoc render agrees empties the row's
  `[post-hoc path differs: …]` bracket, which moves the scorecard and fails
...
* **F-FILLTGT — the fill path's two target decodes disagree on an invalid code.** (Registered 2026-08-03
  ...
  **RETIRED 2026-09-12 (FILL-TGT, cause A5).** "The ROM does not cover the case" was wrong: VDPFIFOTesting
  **test 34 "DMA Fill Control Port Writes" group 3** (ROM `$45B2`, disassembled at `$4898..$4948`) arms a
  4-byte fill at `$40020082`, writes register `$8F02` — which replaces CD1-CD0 with `10`, leaving code `$22`,
  a code that names no write target — then triggers with `$68AC`, and reads `$8000-$800F` back **unchanged**
  (`1122 3344 5566 7788 99aa bbcc ddee ff00`). So the *trigger* guard was the right decode and the body was
  the wrong one. `Vdp::run_fill` now takes the same predicate: it walks its address, consumes its length,
  advances source registers 21/22 and opens its busy window, and writes nowhere. It is **not** an early
  return — group 4 sets no length and fills past 16 bytes, which means group 3's counter did reach 0, and
  group 3's own `btst #1` poll at ROM `$48F6` depends on the busy window opening. `target_of` keeps its
  `_ => Vram` fallback, because it answers "which region does this code name", a different question from
  "may this code write"; nothing now writes on the strength of `target_of` alone. Pinned by
  `vdp::tests::a_fill_whose_code_names_no_write_target_writes_nothing_but_still_runs` and its control
...
* **F-SLOTTABLE — RETIRED 2026-09-15 (parcel P1 FILL-RUN).** `Vdp::H40_SLOT_MCLK` / `H32_SLOT_MCLK` are
  now `const` tables built at compile time by `Vdp::slot_offsets` from the published access indices and the
  per-line access count, so `next_active_slot` does no division at all, and the blanked lines' uniform grids
  (`H40_BLANK_SLOT_MCLK` / `H32_BLANK_SLOT_MCLK`, from `Vdp::blank_offsets`) are tabulated the same way for
  the fill walk. **The condition below is met**: the published indices stay in the source, so
  `active_slot_gaps_follow_the_published_pattern` still checks Kabuto's pattern strings, and two new tests
  assert the derivation rather than the values — `slot_offsets_match_the_access_grid` (entry by entry,
  indices → instants) and `the_slot_table_and_the_arithmetic_agree_at_every_mclk_of_a_line` (the lookup
  against the old arithmetic at every one of a line's 3,420 mclk, both widths), plus
  `the_blank_grids_are_sized_by_the_blanked_slot_rate` tying the grids to `slots_per_line` itself. Every
  currency gate is byte-identical, as the entry required. What made it worth doing now: M1's fill walks
  these same slots once per filled byte — 65,536 of them for a full-VRAM fill — where before only FIFO
  drains probed them. F-SLOTGRID is untouched (the mclk mapping is the same one, now tabulated). The entry
  as it stood:
...
*Narrowed 2026-08-14 for two ROMs.* `System::run_until_with_sink` now honours a sink's `stop_requested`, so a
scraper can name the condition it is waiting for and keep its old number only as a bound.
`m68k_illegal` and `vdp_sprite_masking` are converted (see their rows above); both `assert!(record.fired())`,
so for those two a ROM that slows past its bound is now a **loud failure** rather than a silent misread.
*(2026-09-18: `vdp_sprite_masking`'s stop carries more weight than it used to. Its scrape reads a captured
frame rather than re-rendering settled state, so the stop now also decides **which frame** is scraped —
hence the guard requiring the captured frame to be strictly later than the ROM's last VDP write.)* The
remaining seven budgets are unconverted — a separate, reviewable change.

The conversion is deliberately narrow, because an idle detector is *not* a safe general settle condition here.
`m68k_bcd`, instrumented over its full 700-frame budget, touches the VDP on frames 0, 6 and **530** — 523
frames of complete silence in the middle while it computes, then it prints its results. Any "the screen went
quiet" predicate stops before the answer exists and reads a blank row as a verdict. Each conversion must be
justified by measuring that ROM over its whole old budget, one ROM at a time.
...
*Narrowed 2026-09-18 (POSTHOC-CARRY).* The four constants are now known to be **substrate-independent** —
they classify the live capture and the post-hoc re-render alike, measured across all nine glyphs, which is
what let the scraper change substrate without re-deriving a single one. Two things stay true. The pins are
still *renderer*-dependent, which is the limitation above and is unchanged. And **`TICK_CROSS` classifies
nothing today**: test 3's second sub-case was the only cross in the corpus and it became a tick in
2026-09-16, so that constant is an unexercised classifier — kept because a regression that reintroduces a
cross must read `TICK/CROSS` rather than `UNKNOWN-GLYPH`, but worth knowing that no run checks it.

*Corroborated again 2026-09-18 (TESTROM-H40-HALF), in a mode the constants had never seen.* All nine H40
glyphs classify against the same four hashes with **no re-derivation** — the ROM draws its verdicts from the
same tiles and palette in both modes, so a wider display does not move them. That is now two independent
substrate/mode changes the pins have survived without being touched, which is the standing that makes
"re-derive a constant so the glyph classifies" a BLOCKED item rather than a judgement call: it has twice
looked necessary from a distance and twice been unnecessary on measurement. `TICK_CROSS` still classifies
nothing, in either mode.

*And the L3 hashes are now read through a `ProvenScreen`.* A glyph hash is only meaningful against the
screen it was drawn on, so the classifier takes a mode that has been measured off the captured picture and
cannot be called without one. An `UNKNOWN-GLYPH` and a *right* glyph read off the *wrong* mode are
different failures, and before this parcel the second one had no name.

## Open questions

**Q1 — the `vdp_sprite_masking` H32/H40 toggle responds to `C`, not `Start`.** **CLOSED 2026-09-18
(TESTROM-H40-HALF): the ROM's text is stale relative to its build. Our core is not defective, and nothing
under `crates/oracle-core/src/` was changed to close it.** Full write-up in `docs/2026-09-18-h40-half.md`;
the evidence in three lines:

* The ROM's wait routine at `$0002FA` writes `#$00` to the **Data** register `$A10003`, waits four `nop`s,
  and then spins on `btst #5,$A10003` — first until bit 5 goes high (release), then until it goes low
  (press). The author's intent was plainly "drive TH low, read Start on bit 5".
* **Writing the Data register does not drive TH.** A pin drives the wire only if the **Control** (direction)
  register makes it an output (recon IO2/IO3). And the 4-byte operand `00 A1 00 09` — P1's Control register —
  **does not occur anywhere in the ROM's 256 KB image** (nor does P2's `00 A1 00 0B`); `00 A1 00 03` occurs
  exactly three times, which are the operands of the three instructions above. This ROM never configures TH
  as an output, ever. TH therefore stays an input, floats high on the pull-up (IO3), and bit 5 of the
  TH-high byte is **`C`** (IO4). The ROM reads `C` because `C` is what is on the wire.
* Traced at the instruction: with nothing held and with `Start` held, the read at pc `$000316` returns
  `$FF` and the ROM spins; with `C` held it returns `$DF` (bit 5 clear) and the mode flips. No write to
  `$A10009` appears in the trace either.

**Corroborated in-corpus rather than cross-emulator, which is the sharper arm here.** `vdp_port_access`
*does* write `$A10009` (at `$1126` and `$116E`) and is advanced through all 16 of its pages **by pressing
`Start`** in this same harness, with a pinned `16/0/16`; `io_sample` writes it once and reports `JOYPAD` on
both ports. So this core's `Start` bit is demonstrably readable by a ROM that drives TH. A cross-emulator
...
**Q2 — CLOSED 2026-09-18 (`TESTROM-OPCODE-SIZES-NEVER-SETTLES`).** Q2 said `m68k_opcode_sizes` was
unscraped because "at the pinned 120-frame budget the screen shows the ROM's font/pattern page rather than a
result page, so finding its result page (a longer budget, or an input) is what is deferred". Half right, and
the wrong half is the half that mattered. There is **no font page** — the thing that looked like one is an
identity nametable whose tile indices decode as printable ASCII — so no budget and no input would ever have
produced a result page *in the channel that was being looked at*. The result page was in the tile art all
along, and reaching it needed only a longer budget: the pinned frame is **27.8% through the sweep**,
measured off the ROM's own opcode counter. No input is required at all. The ROM is scraped as of 2026-09-18
and reads `2/2 crc32` against constants baked in its own image; see its row above and
`docs/2026-09-18-opcode-sizes-diagnosis.md`. **With Q2 closed, every ROM in the corpus that has a verdict
now has a readable one.**
...
2. Investigate with `cargo run -p oracle-core --example testrom_probe -- vendor/TestRoms/<rom>.bin <frames>
   ...
   `PRESS`/`PRESS_AT`/`PRESS_LEN`, `PRESSES=<at>:<btn>:<len>,…`, `PRIO_ROWS`, `CPU`, `RAM=<hex>,<count>`,
   `RAM_CRC`/`VRAM_CRC=<hex>,<len>`). For a ROM with more than one
...
4. **Before concluding a ROM has no verdict, check that the channel you are reading can vary at all.**
   `m68k_opcode_sizes` cost eight weeks to a text-grid dump that was decoding an **identity nametable**: the
   cells are `row*40 + col`, written once at boot, so the "text" was constant by construction and agreed at
   every budget for a reason that had nothing to do with the ROM's progress. A surface that cannot vary
   cannot disagree. The cheap instruments for this are now in the probe: `CPU=1` says whether the machine is
   progressing, spinning or parked (and *where* — a PC inside a short backward branch is a wait loop, not a
   hang), `RAM=<hex>,<count>` usually finds the ROM's own progress counter, and `RAM_CRC`/`VRAM_CRC` let you
   recompute a ROM's self-check over our machine instead of taking its printed verdict on faith.
5. **A verdict need not be a word.** `m68k_opcode_sizes` paints `$66666666` (green) or `$99999999` (red);
   `vdp_sprite_masking` draws glyphs; `m68k_illegal` sets the backdrop. A printable-ASCII sweep that finds no
   `PASS` anywhere is evidence about the ROM's alphabet, not about whether it grades itself.
6. If a row is scraped at coordinates, **re-derive them in the new condition** rather than reusing the old
   ones. A derived number that comes out equal to a copied one is fine and should say so; a copied one is
   not. (`vdp_sprite_masking`'s verdict rectangle is x 216..248 in both H32 and H40 — but that is a
   measurement about where the ROM puts its cells, not an inherited constant.)
