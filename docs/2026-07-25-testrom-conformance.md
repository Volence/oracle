...
* *(b) which frame.* The last frame the run **completed** before the idle stop (index 14; the stop fires
  ...
  sentence is replaced by three loud guards in `sprite_masking_glyphs`: exactly one complete frame of
  active lines, at least one frame completed at all, and the captured frame **strictly later** than the
  last frame in which the ROM touched the VDP. Attaching the capture is machine-neutral — measured, both
