...
| **F-SCHEMA-READS-LIVE-EMPYREAN** | 🔴 **REGISTERED 2026-09-02 — the live-tree family, with our own suite's green as the victim.** `crates/oracle-aether/tests/schema_conformance.rs:44-56` (`upstream_schema_path`) walks up from `CARGO_MANIFEST_DIR` probing each ancestor for `empyrean/contract/schema/bus-protocol.schema.json` — **a peer's live working tree**, uncommitted edits included, with `$AETHER_CONTRACT_SCHEMA` the only override. So `the_vendored_schema_is_byte_identical_to_the_upstream_contract` goes **red because a teammate saved a file mid-edit**, and — the worse half — would go **green against a change that was never committed and no other lane can see**. Same shape as the boot-from-`../empyrean/docs/` incident, one layer down: nothing fetches, nothing fails, and the artifact is more convincing rather than less. Found while proving the 2026-09-02 failure was upstream drift (empyrean `82982b7`, §11.31) and not `parcel/player-polish`'s — **the control for that proof was running the test on untouched `main`, where it failed identically.** | Fold into `LIVE-TREE-RESIDUE`, which this **upgrades S→M**: that row was scoped to `tools/aether_smoke.py` and `crates/oracle-core/examples/*` reading aeon's live tree, and the same defect is sitting in a gate. Fix shape: resolve through a **committed revision** (`git -C ../empyrean show <pinned>:contract/…`) or drop the walk-up and require the env var, so an unset path is a loud skip rather than a silent read of someone's desk. `PROVENANCE.md` pinning no tracked revision is the other half — pin one. **↪ ANSWERED 2026-09-02, and my framing was wrong: this is the suite's DEFAULT shape, not ours.** The hub's first enumeration searched for gates reading a contract FILE and found only ours; **sigil corrected it within the hour** — the population to enumerate is *what READS a peer's tree, not what NAMES one*, and by that measure it is everywhere: sigil `test_support.rs:601` `LIVE_TREE_FALLBACK` behind 247 `aeon_dir()` call sites plus three committed scripts (two on active systemd timers), aeon `test.sh:286`, and **our own `crates/oracle-core/examples/common/rom_source.rs:44` `LIVE_AEON_DIR` and `tools/aeon_pin_report.py:145`** — both confirmed at our tip here. Aurora and seraph clean. The resolver case carries a hazard the contract-file case lacks: **the revision moves under a single run**, so a pass is attributable to whatever the tree happened to contain. **Ratified fix shape** (`contract/SUITE_PATHS.md`, empyrean `38f6df4`, verified an ancestor of their `origin/main`, and it cites this finding by name): read the peer **through git objects at a named revision, never the working tree**; vendored bytes **hashed against a blob pinned in a provenance sidecar**; re-vendor via `git -C <peer> show origin/<default>:<path>`; an env-var override is legitimate and **its absence is a loud skip naming the variable, not a walk**. Precedent: aurora's `test/formats/effects-preset-schema-drift.test.ts`. |
