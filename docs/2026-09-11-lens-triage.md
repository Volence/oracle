...
**Updated after HOST-SHUTDOWN-UNLINK (merge `ab1bb82`).** Closed: the hosted path now carries its bind's `(dev, ino)` claim through `Server::listening()` (the one constructor of `Listening`; `into_parts`, `From<UnixListener>` and `spawn_accept`'s `Into` removed), and `Host::shutdown` unlinks nothing, so the player window and `oracle-frontend` remove only the socket file they bound. The stale `main.rs` comment was corrected in the same parcel. **New, found and fixed there:** a `Host` that was shut down could not serve again (`ctx.stop` stayed set; `Host::serve` now clears it after a successful bind); no caller re-serves today. **Residue, named not measured:** a mutation moving the identity check back after the listener closes cannot be caught by a deterministic hosted row (the window sits inside one synchronous `shutdown`); the design closes it structurally, since `Host` holds no identity. A pre-shutdown `wait_for_break` can outlive a re-serve by its own budget on an already-shut socket; harmless, named in the code.
