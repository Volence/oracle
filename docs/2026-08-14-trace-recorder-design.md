...
| ~~`F-SCANLINE-CAPTURE`~~ **CLOSED 2026-08-14** | `scanline_capture::ScanlineCapture { retain: First \| LastFrame \| All }` promoted into `oracle-core`; `BusEventSink::on_frame_boundary(frame)` added (defaulted, forwarded by `&mut S` / `Option<S>` / `Fanout`) and called once per frame from the `Scanline` line-224 event. **Boundary chosen = end of active display, NOT line 0**: `run_frames` deadlines land on a frame-boundary mclk and the loop tests `now < deadline` before popping due events, so a top-of-frame hook would orphan the last frame of every run (this is why a 1-frame run delivers 224 lines, not 225). Both ad-hoc sinks deleted; `frame_hash=0x917371f07409cb25` unmoved. **HARDENED 2026-08-14 (review follow-up)**: the retention oracle ran on `testrom::build()`, whose picture is a constant all-black frame, so it compared black to black and passed under a first-frame-instead-of-last mutation — re-pointed at the vendored `color_1536` (120 frames, the conformance budget) with a first-vs-last non-vacuity guard; `frame_boundary_is_state_neutral` gained an observation assertion (it was green with the hook deleted); and `LastFrame` regained the resync `FrameCapture` had (`if line == 0 { clear }`, generalised to any non-advancing line), which a reset mid-frame otherwise turned into a 324-line "frame" | `src/scanline_capture.rs`, `bus.rs` (`on_frame_boundary`), `system.rs` (`deliver_event`) | no — closed |
| ~~`F-TRACE-EXPOSE-LATCHES`~~ **CLOSED 2026-08-14** | Shipped: `System::z80_busreq()` / `System::z80_running()` / `System::fm()` + `Ym2612::addr_latch(bank)`, read-only, additive, zero currency movement. **The "deletes 3 shadow reimplementations" claim in this row was wrong and is retracted — it deletes 0.** Fable's round-2 §3 corrected it to one deletion (`K4Probe`) plus two structural non-starters (`VgmLogger`/`AudioSink` are *sinks*: handed events, never the machine, on a byte-frozen VGM golden). **Then the one remaining deletion was measured and refuted too.** A sink cannot read an accessor mid-stream, so the only available call site is the frame boundary in `probe()`; the five stateful counters gate on the latch **at each event**, and the guest's take-bus → upload → release handshake completes many times *inside* one frame. Substituting a frame-boundary sample moved **22 of 29 sweep rows**: `zcW!` 0 → 7563 on `aeon-s4` (an ordinary driver upload misread as writes through a closed window), 0 → 1513 on `sonic3k`, 0 → 2286 on `vectorman`; `m68k_memory_test`'s `zcR!` collapsed 3 → 0 while `zw!` fabricated a 1; `1100R!` fabricated a hit on nearly every ROM; and the `wwW!` detail lines went 48 → 32. That table is the evidence behind the shipped K4 open-bus finding, so `K4Probe` **keeps its shadow** — which is the correct implementation, not a duplication. Honest yield: **accessors only, three consumers-in-waiting, zero deletions.** The forward value stands (the next bus-arbitration hunt reaches for these); the census-vs-latches conclusion at `:790-825` is unaffected — it was always about what `K4Probe` *needs*, and a per-event latch is still not a census primitive. Negative result recorded in the probe's own doc comment so it is not re-litigated | `system.rs` (`z80_busreq`/`z80_running`/`fm`), `ym2612.rs` (`addr_latch`), `k4_openbus_probe.rs:45` | no — closed |
