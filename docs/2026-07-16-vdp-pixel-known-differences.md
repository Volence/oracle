...
|---|---|---|---|---|---|
| P1 | **Which dots survive a mid-sprite cut — screen order.** The cut itself is **CLOSED** (2026-09-16): the per-line budget is spent dot by dot, a straddling sprite draws exactly `max_px − px_used` dots and discards the rest (`CutPixelBudget`), the cut sets dot overflow, and the sprite keeps its sprite-count slot. What remains interim is **which** dots survive: we keep the sprite's **leftmost on screen**, h-flipped or not. | R10 / RR8 | RR8 pins the column-major cell order and that hflip mirrors the cell columns — but that is the screen→source *mapping*, not the temporal order of the pattern fetches, so it does not discriminate screen order from **fetch order** (under which an h-flipped straddler would keep its *rightmost* screen dots instead). Nemesis's masking/overflow ROM does not discriminate them either — **measured**, both orders score `3=TICK/TICK`. Interim = the consistent extension of the rule RR8 *does* pin: the sprite is a screen-ordered scan with mirrored source addressing (`Vdp::draw_sprite`), so the budget is spent in the order that scan emits. | `scene_mid_sprite_cut` (two bands, each 240/256 px spent then one 32-px straddler at x 100 with 16 px left — one plain, one h-flipped, and the h-flip must not move the surviving dots). `scene_pixel_budget_exact_fill` (ex-`scene_no_mid_sprite_cut`) pins the exact-fill case beside it. | A hardware/Exodus frame — or any emulator known to model the sprite *fetch slots* — showing an **h-flipped** sprite straddling the budget. The unflipped case no longer needs confirming: Nemesis's ROM confirms it (test 3 `MAX SPRITE DOTS – COMPLEX` now passes). |
...
| P9 | **Atomic DMA fill/copy — no mid-transfer interleave.** A fill/copy mutates VRAM atomically at the trigger; the DMA-busy window models elapsed time, but a data-port write issued *during* the busy window applies **after** the whole fill (hardware: the fill suspends until the FIFO drains and resumes **with the new data**, one location further — with a documented coalescing race). Mid-fill control writes likewise resolve at trigger time via the live registers. | R4(b) (Nemesis, "best working theory"); DMA push Decision 3 | Officially prohibited territory (only PSG/HV/status may be touched mid-fill) AND hardware itself is intermittent on three of these cells — an interleaving model would pin what hardware does not reliably do. | (none — no scene writes mid-fill; recorded model) | A hardware probe if a real game is ever traced doing mid-fill data writes; until then the divergence is attributable here. |
