...
| `a_run_on_no_reported_row_is_in_neither_string_and_changes_nothing` | The boundary: a run painted below the clip leaves the surface **character for character identical**. ⚑ **Amended 2026-09-19, and the digit mask this row shipped with is gone from the comparison.** The two sides are two `fixture()` builds and the Pacing panel prints the loop's own measured rate, so the comparison was reading the box's speed: a raw comparison failed first on `272.73` vs `243.24`, and the digit mask that repaired *that instance* then failed on CI (run `35418036061` on `462e9cf`) on the digit COUNT it cannot see — `#.##` against `###.##`, `### ms` against `#### ms`. Fixed at the clock instead: `fixture` drives `Loop::iterate` at a nominal `FRAME_PERIOD` from its own base, so every `PacingFacts` figure is the same string on any box, the comparison is RAW, and the mask survives only in the failure message. |
