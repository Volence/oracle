...
KNOWN_DIFFERENCES = [
    ...
    # "io-data-bit7" was here until 2026-09-25 -- the one entry that did NOT pin ours as right. CLOSED,
    # not flipped: F-IO-DATA-BIT7 settled that Data-register bit 7 reads back the DATA LATCH's own bit 7
    # (BlastEm's answer), on a die netlist (emu-russia/SEGAChips IOChip/IO.v @ 6ec064e, flip-flop g_87),
    # a decap-derived emulator (Nuked-MD iochip.c @ 9c219b3) and a console measurement (Charles MacDonald,
    # gen-hw.txt v0.8 sec. 3.1). Our core now agrees with BlastEm byte for byte on th_pullup.bin, so there
    # is no divergence left to ledger. docs/2026-09-25-io-data-bit7.md; gate
    # `data_bit_7_follows_the_latch_through_the_bus` in crates/oracle-core/tests/io_controllers.rs.
]
