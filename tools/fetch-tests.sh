...
FILES=(ADD.w SUB.w ADD.b SUB.b ADD.l SUB.l MOVE.w MOVE.b MOVE.l MOVEA.w MOVEA.l Bcc BSR JMP JSR RTS DBcc RTR TRAP RTE TRAPV CHK ANDItoSR ORItoSR EORItoSR RESET CMP.b CMP.w CMP.l CMPA.w CMPA.l TST.b TST.w TST.l CLR.b CLR.w CLR.l MOVE.q ADDA.w ADDA.l SUBA.w SUBA.l AND.b AND.w AND.l OR.b OR.w OR.l EOR.b EOR.w EOR.l NEG.b NEG.w NEG.l NEGX.b NEGX.w NEGX.l NOT.b NOT.w NOT.l EXT.w EXT.l SWAP Scc TAS BTST BCHG BCLR BSET)
